...
const NotFound = () => {
  ...
  const excludedPaths = privateRoute.map((route) => route.path);
  const refreshToken = Cookie.get(REFRESH_TOKEN_KEY);
  const isMatchExcludedPath = excludedPaths.some((path) => matchPath(path, location.pathname));

  useLayoutEffect(() => {
    if (isMatchExcludedPath && !refreshToken) {
